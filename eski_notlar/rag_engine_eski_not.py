...
class IndexLayer:
    ...
    def save_index(self):
        ...
        # FAISS C++ katmani Windows'ta non-ASCII yollari desteklemedigi icin
        # once temp klasorune yaziyoruz, sonra asil yoluna tasiyoruz.
        fd, temp_path = tempfile.mkstemp(suffix=".bin")
        os.close(fd)
        
        faiss.write_index(self.index, temp_path)
        shutil.move(temp_path, FAISS_PATH)
    ...
    def load_index(self) -> bool:
        ...
        try:
            # Okurken de yine once temp klasorune kopyalayip, oradan FAISS'e okutuyoruz.
            fd, temp_path = tempfile.mkstemp(suffix=".bin")
            os.close(fd)
            shutil.copy2(FAISS_PATH, temp_path)
